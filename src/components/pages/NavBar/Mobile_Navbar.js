...
const MobileNavbar = ({ toggleMobileNavbar }) => {
  ...
  return (
    <section className={darkTheme ? "navbar mobile dark" : "navbar mobile"}>
      ...
      <div
        className={
          !darkTheme ? "nav-buttons mobile" : "nav-buttons mobile dark"
        }
      >
        <NavbarButtons />
        <a href="https://docs.google.com/document/d/1nrUwap4LzXaiWbC4XCv2hUr39Pyzet0Pfi6I7uZuAKA/edit?usp=sharing">
          <p className={!darkTheme ? "navbar_resume" : "navbar_resume dark"}>
            Resume
          </p>
        </a>
      </div>
      <button
        className={darkTheme ? "navbar_themebutton dark" : "navbar_themebutton"}
        onClick={() => {
          toggleTheme();
        }}
      >
        {!darkTheme ? (
          <img
            src="https://res.cloudinary.com/ducqdbpaw/image/upload/v1764456478/Group_15_h3zpfj.svg"
            alt="crescent moon icon for dark mode"
          />
        ) : (
          <img
            src="https://res.cloudinary.com/ducqdbpaw/image/upload/v1764456472/Group_14_vbyb9p.svg"
            alt="crescent moon icon for dark mode"
          />
        )}
      </button>
